...
void printHelp() {
    std::cout << "Usage: " << PROGRAM_NAME << " [OPTIONS]\n\n"
              ...
              << "  --plugin-security LEVEL  Set plugin security (default/restricted/permissive)\n"
              << "                           Default: restricted (production-safe)\n"
              << "  --no-plugin-sandbox Disable plugin sandboxing (NOT RECOMMENDED)\n"
              << "  --no-verify-signatures  Disable GPG signature verification (UNSAFE)\n"
              << "\n"
              << "Security:\n"
              ...
              << "\n"
              << "SECURITY NOTES:\n"
              << "  - Plugins require GPG signature verification by default (.so.sig files)\n"
              << "  - Default security policy is 'restricted' for safety\n"
              << "  - Running as root requires explicit --allow-root flag\n"
              << "  - Use --no-verify-signatures only for development/testing\n"
              << "\n"
              << "Examples:\n"
              ...
              << "  " << PROGRAM_NAME << " --plugin ./myplugin.so # Load signed plugin\n"
              << "  " << PROGRAM_NAME << " --plugin-dir /opt/plugins # Load all signed plugins\n"
              << "  " << PROGRAM_NAME << " --plugin-security permissive # Use relaxed security\n"
